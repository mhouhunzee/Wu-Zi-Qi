...
class FastTrainer:
        ...
        for game_idx in range(num_games):
            ...
            while not done:
                ...
                if current_player == agent_color:
                    action = agent.get_action(env, epsilon=epsilon, use_rules=False)
                else:
                    action = opponent_agent.get_action(env, epsilon=epsilon, use_rules=False)
    ...
    def train_cycle(self, cycle_num, t=T):
        ...
        # 获取White的对比基准（上一个Cycle训练Black时White的胜率）
        if cycle_num > 0:
            prev_cycle = self.stats['cycle'][-1]
            prev_black_results = prev_cycle['black_training']['results']
            self.white_pre_train_win_rate = self._calculate_win_rate(prev_black_results, WHITE)
            print(f"  [基准] White对比基准（上轮Black训练阶段White胜率）: {self.white_pre_train_win_rate:.2%}")
        else:
            self.white_pre_train_win_rate = None
        ...
        # 获取Black的对比基准（当前White训练阶段Black的胜率）
        white_training_results = cycle_stats['white_training']['results']
        self.black_pre_train_win_rate = self._calculate_win_rate(white_training_results, BLACK)
        print(f"  [基准] Black对比基准（当前White训练阶段Black胜率）: {self.black_pre_train_win_rate:.2%}")
        
        # 评估White训练后的表现（对比基准：上一轮Black训练阶段White的胜率）
        if cycle_num > 0 and self.white_pre_train_win_rate is not None:
            white_post_train_win_rate = self._calculate_win_rate(white_training_results, WHITE)
            print(f"\n  [评估] White训练后胜率: {white_post_train_win_rate:.2%} (对比基准: {self.white_pre_train_win_rate:.2%})")
            
            if white_post_train_win_rate < self.white_pre_train_win_rate:
                print(f"  [保护] White训练后表现({white_post_train_win_rate:.2%})不如对比基准({self.white_pre_train_win_rate:.2%})，回退模型")
                self._rollback_model(self.white_agent, 'white', cycle_num + 1)
            else:
        ...
        # 评估Black训练后的表现（对比基准：当前White训练阶段Black的胜率）
        black_training_results = cycle_stats['black_training']['results']
        if self.black_pre_train_win_rate is not None:
            black_post_train_win_rate = self._calculate_win_rate(black_training_results, BLACK)
            print(f"\n  [评估] Black训练后胜率: {black_post_train_win_rate:.2%} (对比基准: {self.black_pre_train_win_rate:.2%})")
            
            if black_post_train_win_rate < self.black_pre_train_win_rate:
                print(f"  [保护] Black训练后表现({black_post_train_win_rate:.2%})不如对比基准({self.black_pre_train_win_rate:.2%})，回退模型")
                self._rollback_model(self.black_agent, 'black', cycle_num + 1)
            else:
